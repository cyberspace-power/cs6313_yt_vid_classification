...
samples = 4940  # This is how many we have from category 1 (fewest of any category)
...
flag = True if samples - new_text_df.shape[0] <= 50 else False  # Decides whether to manually select data or not
...
for file in image_files:  # Iterate through shuffled file names
    ...
    if file[:-4] in set(full_text_df['Video Id']) and file[:-4] not in set(new_text_df["Video Id"]):
        ...
        if flag:
            choice = input(str(count) + ": Do you want to add the following data point:\n\tVideo Id:\t" + file[:-4] +
